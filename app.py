...
class VendorRequest(db.Model):
    ...
    # --- ADDRESS (New from CSV) ---
    contact_name = db.Column(db.String(100))
    contact_number = db.Column(db.String(20))
    telephone_number = db.Column(db.String(20)) # New
    contact_designation = db.Column(db.String(100))
    
    address_line1 = db.Column(db.String(200)) # New
    address_city = db.Column(db.String(100))  # New
    address_state = db.Column(db.String(100)) # New
    address_pincode = db.Column(db.String(20)) # New

    # --- COMMERCIAL (Internal Use) ---
    payment_terms = db.Column(db.String(100)) # New
    incoterms = db.Column(db.String(100))     # New
    purchase_org = db.Column(db.String(50))   # New
...
def send_system_email(to, subject, body, link=None):
    ...
    try:
        ...
        # Use template if available, else fallback to string
        try:
...
def vendor_portal(token):
    # Use db.session.get for safety, or filter_by
    req = VendorRequest.query.filter_by(token=token).first_or_404()
    
    if req.status != 'PENDING_VENDOR': 
        return render_template('success.html', req=req) # Or a "Form Closed" page
