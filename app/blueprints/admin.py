...
# # --- 1. UNIFIED API FOR UPDATES (AJAX) ---
# @admin_bp.route('/api/update', methods=['POST'])
# @login_required
# def api_update():
#     """Handles all Create/Update/Delete actions for Workflow, Users, and Masters."""
#     data = request.get_json()
...
#                 u.email = data['email']
#                 u.department = data['dept']
#                 u.role = data['role']
#                 u.assigned_category = data['category']
#             else:
...
#             if m_id:
#                 m = db.session.get(MasterData, m_id)
#                 m.code = data['code']
#                 m.label = data['label']
#                 m.is_active = data.get('is_active', True)
#             else:
#                 if not MasterData.query.filter_by(category=cat_code, code=data['code']).first():
#                     db.session.add(MasterData(category=cat_code, code=data['code'], label=data['label']))
...
#     """Fetch workflow rules for a department."""
...
#     """Fetch master data items by slug."""
#     SLUG_TO_DB = {
#         'region': 'REGION', 'payment-terms': 'PAYMENT_TERM', 'inco-terms': 'INCOTERM',
#         'msme-type': 'MSME_TYPE', 'account-group': 'ACCOUNT_GROUP', 'gl-list': 'GL_ACCOUNT',
#         'house-bank': 'HOUSE_BANK', 'purch-org': 'PURCHASE_ORG', 'tds-types': 'TAX_TYPE',
...
#     # Handle New Dept Creation (SSR is fine for this, rare action)
...
#     # Serialize ALL data for the client
#     app_data = {
#         'active_tab': request.args.get('active_tab', 'dashboard'),
#         'selected_dept': request.args.get('selected_dept', 'Purchase'),
#         'stats': admin_service.get_dashboard_stats(),
...
@login_required
def api_update():
    """Unified endpoint for all Admin actions."""
    if current_user.role != 'admin': return jsonify({'error': 'Unauthorized'}), 403
    ...
    try:
        ...
        # ==========================================
        if action == 'rename_dept':
            dept = Department.query.filter_by(name=data['old_name']).first()
            if dept and data.get('new_name'): 
                dept.name = data['new_name']
                # Manual cascade for consistency
                CategoryRouting.query.filter_by(department=data['old_name']).update({'department': data['new_name']})
                WorkflowStep.query.filter_by(department=data['old_name']).update({'department': data['new_name']})
                User.query.filter_by(department=data['old_name']).update({'department': data['new_name']})
        
        elif action == 'delete_dept':
            dept = Department.query.filter_by(name=data['name']).first()
            if dept: 
                db.session.delete(dept)
                CategoryRouting.query.filter_by(department=data['name']).delete()
                WorkflowStep.query.filter_by(department=data['name']).delete()

        # ==========================================
        # 2. FINANCE & IT ASSIGNMENTS
        # ==========================================
            ...
            if step:
                ...
                db.session.add(WorkflowStep(department='Finance', role_label=data.get('id'), approver_email=data.get('email'), step_order=0))

        elif action == 'it_route':
        ...
        # ==========================================
        # 4. USER MANAGEMENT (VALIDATED)
        # ==========================================
        elif action == 'save_user':
            u_name = data.get('name')
            u_email = data.get('email')
            u_dept = data.get('dept')
            u_role = data.get('role')
            u_cat = data.get('category')

            # VALIDATION: Check Name and Email
            if not u_name or not u_email:
                return jsonify({'success': False, 'error': 'User Name and Email are mandatory fields.'}), 400
        ...
        # ==========================================
            ...
            db_cat = SLUG_TO_DB.get(cat_slug) or data.get('category_code')

            if data.get('id'):
                m = db.session.get(MasterData, data['id'])
                if m: m.code, m.label, m.is_active = data['code'], data['label'], data.get('is_active', True)
            else:
...
def admin_workflow():
    ...
    master_types = [
        {'slug': k, 'label': k.replace('-', ' ').title()} 
        for k in ['region', 'payment-terms', 'inco-terms', 'msme-type', 'account-group', 'gl-list', 'house-bank', 'purch-org', 'tds-types', 'tds-codes']
    ]
...
def export_sap_data():
    ...
    # We fetch IDs of all completed requests to pass to the service
